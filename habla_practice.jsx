...
const TOPICS = [
  { cat: "Everyday Life", items: ["How you make your morning coffee or tea","What your neighborhood looks like","Your typical weekday routine","How to do laundry from start to finish","What's in your fridge right now and why","How you get to work or school","Your favorite meal to cook and how you make it","What you do to relax after a long day","How you organize your phone apps","Your weekend routine versus your weekday routine"] },
  { cat: "People & Relationships", items: ["Describe your best friend's personality","What makes a good neighbor","How you met someone important in your life","What your family dynamic is like","The difference between a good boss and a bad boss","How friendships change as you get older","Describe someone you admire and why","What you look for in a travel companion","How people in your culture greet each other","The funniest person you know and what makes them funny"] },
  { cat: "How Things Work", items: ["How a refrigerator keeps food cold","Why the sky changes color at sunset","How a search engine finds results","Why we dream when we sleep","How a bicycle stays balanced","What happens when you catch a cold","How plants turn sunlight into food","Why some things float and others sink","How a microwave heats food","What WiFi actually is"] },
  { cat: "Opinions", items: ["The best season of the year and why","City vs countryside living","Your favorite type of music and why","Cooking at home vs eating out","The most overrated thing in popular culture","Morning vs night productivity","Your unpopular opinion about food","Social media: more good or harm?","The ideal age to learn a second language","Pets or no pets?"] },
  { cat: "Describe & Compare", items: ["Your country's weather vs Spain's","Compare two cities you've visited","A book vs its movie adaptation","How you studied as a teen vs now","Being busy vs being productive","Compare two sports you know","Your life five years ago vs now","Being alone vs being lonely","Compare two holidays you enjoy","Talent vs hard work"] },
  { cat: "Hypothetical", items: ["What you'd do with an extra hour daily","If you could live in any historical period","Your perfect no-budget vacation","If you had to teach a class on anything","What you'd change about your city","Dinner with anyone alive — who?","Explain your job to a child","Instantly master one skill — which?","The world in 50 years","Redesign the school system"] },
  { cat: "Culture & Society", items: ["A tradition from your culture","How holidays are celebrated where you're from","Why people travel","The role of music in daily life","How fashion has changed in your lifetime","What makes a city feel alive","Local markets vs big stores","How humor differs across cultures","Food bringing people together","Why people are nostalgic"] },
  { cat: "Storytelling", items: ["The last dream you remember","A time you got completely lost","The most memorable meal you've had","A creative problem-solving moment","Your most embarrassing moment","A trip that didn't go as planned","First time trying something scary","A moment that changed your perspective","The strangest coincidence you've had","Communicating without a shared language"] },
  { cat: "Abstract", items: ["What it means to be brave","Whether people can truly change","What makes something beautiful","Knowledge vs wisdom","Why time feels faster with age","What success means to you","Are rules necessary for freedom?","What makes a memory stick?","Language and thought","Happiness: choice or circumstance?"] },
];
...
const CHUNKS = [
  ...
  { cat: "Lifting Weights", items: [
    ...
    { es: "Estoy haciendo sentadillas / peso muerto / press de banca", en: "Squats / deadlifts / bench press" },
    { es: "Descanso un minuto entre series", en: "I rest one minute between sets" },
    { es: "Me estoy estancando, necesito cambiar la rutina", en: "I'm plateauing, I need to change my routine" },
  ]},
  { cat: "Food Prep", items: [
  ...
  { cat: "Food Order", items: [
    ...
    { es: "Me he equivocado, quería la otra cosa", en: "I made a mistake, I wanted the other thing" },
    { es: "¿Tenéis algo para celíacos?", en: "Do you have anything for celiacs?" },
  ]},
  { cat: "Walking the City", items: [
  ...
  { cat: "Tax Advisor", items: [
    { es: "Tengo que hacer la declaración de la renta", en: "I need to file my income tax return" },
  ...
  { cat: "Software Presentation", items: [
    { es: "Os voy a enseñar cómo funciona la plataforma", en: "I'm going to show you how the platform works" },
    ...
    { es: "Esto se integra con las herramientas que ya usáis", en: "This integrates with your existing tools" },
    { es: "Aquí os muestro un caso de uso real", en: "Here I'll show you a real use case" },
    { es: "¿En qué se diferencia de la competencia?", en: "How does it differ from the competition?" },
  ]},
];
...
// --- Storage helpers using window.storage ---
async function loadData() {
  try {
    const r = await window.storage.get("habla-data");
    return r ? JSON.parse(r.value) : { sessions: [], streak: 0, lastDate: null };
  } catch { return { sessions: [], streak: 0, lastDate: null }; }
}
async function saveData(data) {
  try { await window.storage.set("habla-data", JSON.stringify(data)); } catch(e) { console.error(e); }
}
...
const StopIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><rect x="4" y="4" width="16" height="16" rx="2"/></svg>;
...
const ChevronIcon = ({down}) => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{transform:down?"rotate(0)":"rotate(-90deg)",transition:"transform 0.2s"}}><polyline points="6 9 12 15 18 9"/></svg>;
...
export default function App() {
  ...
  const randomPrompt = useCallback(() => {
    ...
    setAudioUrl(null); setShowRating(false); setElapsed(0); setShowEn({});
  }, [mode]);

  useEffect(() => { if (loaded && !prompt) randomPrompt(); }, [loaded, randomPrompt, prompt]);
  ...
  if (!loaded) return <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:"100vh",fontFamily:"'DM Sans',sans-serif",color:"var(--text-secondary, #888)"}}>Loading...</div>;
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .rating-btn {
          ...
          padding: 14px 18px; border-radius: 12px; border: 2px solid var(--card-border);
      ...
      <div style={{ marginBottom: 20, display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Activate your Spanish vocabulary</p>
        </div>
        <div style={{ textAlign: "right" }}>
          <div className="mono" style={{ fontSize: 22, fontWeight: 500, color: "var(--accent)" }}>{data.streak}</div>
          <div style={{ fontSize: 10, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.5 }}>day streak</div>
        </div>
      </div>
      ...
      <div className="tab-bar">
        ...
        <button className={`tab ${view==="log"?"active":""}`} onClick={()=>setView("log")}>Progress</button>
      </div>
      ...
      {view === "practice" && <>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          <button className={`btn btn-sm ${mode==="topics"?"btn-accent":"btn-ghost"}`} onClick={()=>{setMode("topics");setPrompt(null);}}>Explain It Simply</button>
          <button className={`btn btn-sm ${mode==="chunks"?"btn-accent":"btn-ghost"}`} onClick={()=>{setMode("chunks");setPrompt(null);}}>Chunk Drill</button>
        </div>
        ...
        <div className="card" style={{ position: "relative" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <button className="btn btn-sm btn-ghost" onClick={randomPrompt} style={{ padding: "5px 10px" }}>
              <ShuffleIcon /> New
            </button>
          </div>

          {prompt?.type === "topic" && (
            <p style={{ fontSize: 18, fontWeight: 500, lineHeight: 1.4, color: "var(--text-primary)" }}>{prompt.text}</p>
          )}
            ...
            <div>
              {chunkCat.items.map((item, i) => (
                <div key={i} className="chunk-item" onClick={() => toggleEn(i)}>
                  <p style={{ fontSize: 16, fontWeight: 500, color: "var(--text-primary)" }}>{item.es}</p>
                  {showEn[i] && <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4, fontStyle: "italic" }}>{item.en}</p>}
                </div>
              ))}
              <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>Tap a phrase to reveal its English translation</p>
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ textAlign: "center" }}>
          <div className="mono" style={{ fontSize: 42, fontWeight: 500, color: recording ? "var(--accent)" : "var(--text-primary)", marginBottom: 16 }}>
            <span className={recording ? "pulse" : ""}>{fmtTime(elapsed)}</span>
          </div>

          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            {!recording && !audioUrl && (
              <button className="btn btn-accent" onClick={startRec}><MicIcon /> Start Recording</button>
            )}
            {recording && (
        ...
        {showRating && (
          <div className="card" style={{ textAlign: "center" }}>
            <p style={{ fontSize: 14, fontWeight: 500, marginBottom: 14, color: "var(--text-secondary)" }}>How did that feel?</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              {RATINGS.map((r, i) => (
                <button key={i} className="rating-btn" onClick={() => saveSession(r.label)}>
                  <span style={{ fontSize: 26 }}>{r.emoji}</span>
                  <span style={{ fontSize: 12, fontWeight: 500, color: r.color }}>{r.label}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {todaySessions.length > 0 && (
          <div style={{ textAlign: "center", padding: "10px 0", color: "var(--text-muted)", fontSize: 13 }}>
            Today: {todaySessions.length} session{todaySessions.length>1?"s":""} · {fmtTime(todayMin)} total
          </div>
        )}
      </>}
      ...
      {view === "browse" && <>
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          <button className={`btn btn-sm ${mode==="topics"?"btn-accent":"btn-ghost"}`} onClick={()=>setMode("topics")}>Topics</button>
          <button className={`btn btn-sm ${mode==="chunks"?"btn-accent":"btn-ghost"}`} onClick={()=>setMode("chunks")}>Chunks</button>
        </div>

        {mode === "topics" && TOPICS.map((cat, ci) => (
          <div key={ci} className="card" style={{ padding: "6px 20px" }}>
            ...
            {expandedCats[cat.cat] && cat.items.map((item, i) => (
              <div key={i} style={{ padding: "8px 0", borderTop: "1px solid var(--card-border)", fontSize: 14, color: "var(--text-secondary)" }}>{item}</div>
            ))}
          </div>
        ))}

        {mode === "chunks" && CHUNKS.map((cat, ci) => (
          <div key={ci} className="card" style={{ padding: "6px 20px" }}>
            <div className="cat-header" onClick={() => toggleCat(cat.cat)}>
              <span style={{ fontWeight: 600, fontSize: 14 }}>{cat.cat}</span>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>{cat.items.length} phrases</span>
            </div>
            {expandedCats[cat.cat] && cat.items.map((item, i) => (
              <div key={i} className="chunk-item" onClick={() => toggleEn(`b-${ci}-${i}`)}>
                <p style={{ fontSize: 14, fontWeight: 500, color: "var(--text-primary)" }}>{item.es}</p>
                {showEn[`b-${ci}-${i}`] && <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3, fontStyle: "italic" }}>{item.en}</p>}
              </div>
            ))}
          </div>
        ))}
      ...
      {view === "log" && <>
        <div className="card">
          <div className="stat-grid">
            ...
            <div className="stat-box">
              <div className="stat-num">{fmtTime(data.sessions.reduce((a,s)=>a+s.duration,0))}</div>
        ...
        <div className="card">
          ...
          {data.sessions.length === 0 && (
            <p style={{ color: "var(--text-muted)", fontSize: 14 }}>No sessions yet. Start practicing!</p>
          )}
          {[...data.sessions].reverse().slice(0, 20).map((s, i) => (
            <div key={i} className="log-row">
              <div style={{ fontSize: 22 }}>{RATINGS.find(r=>r.label===s.rating)?.emoji || "🤔"}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: "var(--text-primary)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {s.cat}
                </div>
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{s.date} · {fmtTime(s.duration)} · {s.mode}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {data.sessions.length > 0 && (
          <button className="btn btn-ghost btn-sm" style={{ margin: "10px auto", display: "block" }} onClick={async () => {
